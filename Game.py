...
class TicTacToe:
    def __init__(self,training=False):
        # Initialize game setup (existing code)
        self.training = training
        self.win_history = []  # Lưu trữ tỷ lệ thắng
    ...
    def train(self, iterations):
        if self.training:
            for i in range(iterations):
                ...
                isX = random.choice([True, False])  # Chọn ngẫu nhiên người chơi X
                while not done:
                    # Player 1 hoặc 2 thực hiện bước đi dựa trên epsilon-greedy
                    if isX:
                    ...
                    # Thực hiện bước và nhận phần thưởng
                    reward, done = self.step(isX, move)

                    if reward == 1:  # Nếu có người thắng
                        if isX:
                            self.player1.updateQ(reward, self.board, self.possible_moves())
                            self.player2.updateQ(-1 * reward, self.board, self.possible_moves())
                            self.win_history.append(1)  # Player1 thắng
                        else:
                            self.player1.updateQ(-1 * reward, self.board, self.possible_moves())
                            self.player2.updateQ(reward, self.board, self.possible_moves())
                            self.win_history.append(0)  # Player1 thua
                    elif reward == 0.5:  # Hòa
                        self.player1.updateQ(reward, self.board, self.possible_moves())
                        self.player2.updateQ(reward, self.board, self.possible_moves())
                        self.win_history.append(0.5)  # Trận hòa
                    elif reward == -5:  # Di chuyển không hợp lệ
                        if isX:
                            self.player1.updateQ(reward, self.board, self.possible_moves())
                        else:
                            self.player2.updateQ(reward, self.board, self.possible_moves())
                    elif reward == 0:  # Trận đấu chưa kết thúc
                        if isX:
                            self.player2.updateQ(reward, self.board, self.possible_moves())
                        else:
                            self.player1.updateQ(reward, self.board, self.possible_moves())

                    isX = not isX  # Chuyển lượt chơi

    def plot_results(self):
        # Tính tỷ lệ thắng trung bình theo cửa sổ di động (moving average)
        N = 1000  # Kích thước cửa sổ
        cumsum = [0]
        moving_avg = []
        for i, x in enumerate(self.win_history, 1):
            cumsum.append(cumsum[i-1] + x)
            if i >= N:
                moving_avg.append((cumsum[i] - cumsum[i-N]) / N)
            else:
                moving_avg.append(0)
        ...
        plt.title('Training Progress (Player 1)')
        plt.show()
